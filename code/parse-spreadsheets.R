...
get_sheet <- function(path, pattern) {
  if (
    any(
      sheet_skiplist$file == basename(path) & sheet_skiplist$pattern == pattern
    )
  ) {
  ...
  eofy_year <- if_else(
    !is.na(eofy_year) & eofy_year < 100L,
    eofy_year + 2000L,
    eofy_year
  )
...
extract_eid <- function(path, pattern) {
  if (!"Footnotes" %in% excel_sheets(path)) {
    return(NA_character_)
  }
  foot <- tryCatch(
  ...
  dates <- mdy(
    str_extract(col_b[hits], "(?<=EID as of )\\d{1,2}/\\d{1,2}/\\d{4}"),
    quiet = TRUE
  )
...
derive_file_date <- function(file, fy_int) {
  ...
  if (!is.na(yp)) {
    return(ymd(yp, quiet = TRUE))
  }
  ds <- file |>
    str_extract_all("\\d{4,8}") |>
...
file_pull_dates <-
  ...
  map_dfr(
    \(.x) {
      ...
      tibble(
        ...
        eid_detained = extract_eid(
          .x,
          "ICE Currently Detained Population Breakdown"
        )
      )
    },
    .id = "file"
  ...
  mutate(
    ...
    pull_date_facilities = str_extract(src, "\\d{1,2}/\\d{1,2}/\\d{4}") |>
      mdy(),
  ...
  full_join(
    ...
    stay_wide |>
      select(
        agency,
        criminality,
        avg_stay_length_days_fy_ytd = `FY Overall`,
        fiscal_year,
        file_date,
        pull_date
      ),
...
facility_alos <-
  ...
  pivot_longer(
    matches("^fy\\d+_alos$"),
    names_to = "alos_fiscal_year",
    values_to = "alos"
  ) |>
  filter(!is.na(alos)) |>
  mutate(
    alos_fiscal_year = parse_fy(alos_fiscal_year),
...
currently_detained_by_disposition <-
  ...
  select(-file) |>
  pivot_longer(
    any_of(c("fsc_frc", "adult", "total")),
    names_to = "facility_type",
    values_to = "n_detained",
    values_drop_na = TRUE
  ) |>
  mutate(
    facility_type = recode(
      facility_type,
      fsc_frc = "FSC/FRC",
      adult = "Adult",
      total = "Total"
  ...
  safe_map(fls, \(.x) {
    extract_table(
      ...
      post = \(df) {
        ...
        for (col in setdiff(c("fsc", "adult", "total"), positional)) {
          df[[col]] <- NA_real_
        }
        df |>
    ...
    extract_table(
      ...
      post = \(df) {
        ...
        label_col <- detect_index(df, \(c) {
          any(
            as.character(c) %in% c("Total", "FSC", "FRC", "Adult"),
            na.rm = TRUE
          )
        })
        if (label_col == 0 || label_col >= ncol(df)) {
          return(NULL)
        }
        count_col <- label_col +
          detect_index(df[(label_col + 1):ncol(df)], \(c) {
            any(!is.na(suppressWarnings(as.numeric(c))))
          })
        if (count_col == label_col) {
          return(NULL)
        }
        tibble(
...
iclos_and_detainees <-
  safe_map(fls, \(.x) {
    ...
    map_dfr(names(sections), \(section_name) {
      ...
      years <- if_else(
        !is.na(years_raw) & str_detect(years_raw, "^\\d{4}$"),
        years_raw,
        NA_character_
      ) |>
      ...
      data_cols <- which(
        months_raw %in% month.name & periods_raw %in% c("mid", "end")
      )
      ...
      if (section_name == "detainees") {
        is_bucket <- str_detect(
          replace_na(row_labels, ""),
          DETAINEE_BUCKET_PATTERN
        )
        pop_type <- if_else(
          !is.na(row_labels) & !is_bucket,
          row_labels,
          NA_character_
        ) |>
          accumulate(\(prev, cur) coalesce(cur, prev))
  ...
  mutate(
    ...
    population_type = population_type |>
      str_replace_all(
        "^Adult Facility Aliens$",
        "Adult Facility Individuals"
      ) |>
      str_replace_all(
        "^FRC Facility Individuals$",
...
nanoparquet::write_parquet(
  movements_by_facility_type,
  "data/movements-by-facility-type.parquet"
)

removals <-
  full_join(
    distinct(removals_total),
